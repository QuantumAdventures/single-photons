...
class Particle:
    def __init__(self, omega, gamma, coupling, eta_detection=1, radius=147e-9, rho=2200):  
        ...
        self.zp_x = np.sqrt(hbar/(omega*self._m_))
        self.zp_p = np.sqrt(omega*hbar*self._m_)
    ...
    def step(self, states, control=0.0, delta_t=50e-2):
        ...
        states += state_dot*delta_t + self.G*np.sqrt(delta_t)*(thermal_force+backaction_force)
        return states
...
class KalmanFilter:
    ...
    def propagate_dynamics(self, inputValue):
        xk_minus = (
            np.matmul(self.A, self.estimates_aposteriori[self.t_i])
            + self.B * inputValue
        )
        Pk_minus = (
            np.matmul(
                self.A,
                np.matmul(self.error_covariance_aposteriori[self.t_i], (self.A.T)),
            )
            + self.Q
        )
    ...
    def compute_aposteriori(self, currentMeasurement):
        Kk = np.matmul(
            self.error_covariance_apriori[self.t_i - 1],
            np.matmul(
                self.C.T,
                np.linalg.pinv(
                    self.R
                    + np.matmul(
                        self.C,
                        np.matmul(
                            self.error_covariance_apriori[self.t_i - 1], (self.C.T)
                        ),
                    )
                ),
            ),
        )
        error_k = currentMeasurement - np.matmul(
            self.C, self.estimates_apriori[self.t_i - 1]
        )
